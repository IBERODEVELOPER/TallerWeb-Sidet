...
import com.ibero.demo.repository.ActivityDao;
...
public class ControllerMaster {
	...
	public String showIndex(Model model, Authentication authentication) {
        ...
        if(hasRole("ROLE_SUPPORT")||hasRole("ROLE_EMPLOYEE")||hasRole("ROLE_MANAGER")||hasRole("ROLE_ADMIN")) {
            // Buscar el empleado
    	    EntityEmployee employee = peopleService.getEmployeeWithFullSchedule(usere);
	...
	public String sendRestartPass(@RequestParam("mailTo") String mailTo, RedirectAttributes flash) {
		Optional<EntityEmployee> usuarioOpt = peopleService.findByEmailPeople(mailTo);
		if (usuarioOpt.isPresent()) {
			EntityEmployee empl = usuarioOpt.get();
