...
public class LoginSuccesHandler extends SimpleUrlAuthenticationSuccessHandler {
			...
			try {
				...
				if (hasRole("ROLE_ADMIN")||hasRole("ROLE_MANAGER")) {
					...
					if (session.getAttribute("asistenciaRegistrada") == null) {
						EntityEmployee employee = employeeService.getEmployeeWithFullSchedule(userentity);
						if (employee != null) {
	...
	private boolean verificarHorario(HttpServletRequest request, HttpServletResponse response,
		...
		EntityEmployee employee = employeeService.getEmployeeWithFullSchedule(usere);
		if (employee != null) {
	...
	// registrar asistencia.
	public void registrarAsistencia(EntityEmployee employee) {
		Calendar calendar = Calendar.getInstance();
		int horaActual = calendar.get(Calendar.HOUR_OF_DAY);
		int minutosActuales = calendar.get(Calendar.MINUTE);
		int diaSemana = calendar.get(Calendar.DAY_OF_WEEK);
